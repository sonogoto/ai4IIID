...
def stl2voxel(file_path, resolution=98):
    mesh = list(stl_reader.read_stl_verticies(file_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument('input', nargs='?', type=lambda s:file_choices(('.stl', '.STL'),s))
    args = parser.parse_args()
    # doExport(args.input, args.output, 100)
    vol = stl2voxel(args.input)
    print(vol.shape)
    print(vol.dtype)
